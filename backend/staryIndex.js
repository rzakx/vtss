...
const wczytajInfoTelemetria = async () => {
	...
	db.query("SELECT id, name, stawka FROM stawkiPaliwowe WHERE gra = 0", (erst, rst) => {
		if(erst){
	...
	//stawki paliwowe ATS
	db.query("SELECT id, name, stawka FROM stawkiPaliwowe WHERE gra = 1", (erst2, rst2) => {
		if(erst2){
...
const smtp = nodemailer.createTransport({
	...
	dkim: {
		...
		keySelector: "420",
		privateKey: `-----BEGIN RSA PRIVATE KEY-----
		
		n46JiKiEe78NOfE5qsDi2H8NGokGyJ1pDYpvNdQ74+I7
		-----END RSA PRIVATE KEY-----`
	}
});
...
app.post("/kartaPaliwowa/:login", (req, res) => {
	if(!req.params.login){
		res.send({response: true, blad: "Niepoprawny profil"});
		return;
	}
	db.query("SELECT SUM(`kwota`) as 'k', SUM(`litry`) as 'l' FROM `telemetriaPaliwo` WHERE `kierowca_id` = (SELECT `id` FROM `konta` WHERE `login` = ?)", [req.params.login], (er, r) => {
		if(er){
			console.log(er);
			res.send({response: true, blad: "Błąd SQL"});
			return;
...
app.post("/nadajUpr/:token", (req, res) => {
	...
	db.query("SELECT `id` as 'i', `login` as 'l', `discord` as 'dc' FROM `konta` WHERE `token` = ? AND `typkonta` <= 3", [req.params.token], (er, r) => {
		if(r.length > 0){
			...
			db.query("INSERT INTO `uprawnienia` (`instruktor`, `kto`, `naco`, `gra`, `cena`, `odkiedy`, `dokiedy`, `cenaFirmy`) VALUES ?", sqlFormat, async (er2, r2) => {
				if(er2){
					console.log(er2);
...
app.post("/usunKonto/:token", (req, res) => {
	...
	db.query("DELETE FROM `telemetriaPowiazania` WHERE `uzytkownik` = (SELECT `login` FROM `konta` WHERE `id` = ?)", [req.body.idosoby], (ertp, rtp) => {
		db.query("DELETE FROM `telemetriaInformacje` WHERE `login` = (SELECT `login` FROM `konta` WHERE `id` = ?))", [req.body.idosoby], (ertei, rtei) => {
			//winiety
...
app.post("/tokenTelemetria/:token", (req, res) => {
	db.query("SELECT `login`, `discord` FROM `konta` WHERE `token` = ?", [req.params.token], (er, r) => {
		if(er){
		...
		if(r.length > 0){
			...
			const tokenik = CryptoJS.HmacSHA1(saltToken, KLUCZ_H).toString();
			db.query("SELECT `uzytkownik` FROM `telemetriaPowiazania` WHERE `uzytkownik` = ?", [r[0]['login']], async (er2, r2) => {
			// znajdz w telemetriaPowiazania czy istnieje juz login
				...
				if(!r2.length){
					// jesli nie to dodaj
					db.query("INSERT INTO `telemetriaPowiazania` (`uzytkownik`, `token`, `wersja`) VALUES (?, ?, ?)", [r[0]['login'], tokenik, process.env.WERSJA_TELEMETRI], async (er3, r3) => {
						if(er3){ console.log("błąd", er3); }
					...
					// jesli ta to update
					db.query("UPDATE `telemetriaPowiazania` SET `wersja` = ?, `token` = ? WHERE `uzytkownik` = ?", [process.env.WERSJA_TELEMETRI, tokenik, r[0]['login']], async (er3, r3) => {
						if(er3) console.log("błąd", er3);
						res.send({odp: tokenik});
...
app.post("/sprawdzWersjeTelemetria/:token", (req, res) => {
	db.query("SELECT `wersja` FROM `telemetriaPowiazania` WHERE `uzytkownik` = (SELECT `login` FROM `konta` WHERE `token` = ?)", [req.params.token], (er, r) => {
		if(er){
...
app.post("/dostanAwatary/", (req, res) => {
	let tmp = [];
	db.query("SELECT login, awatar FROM konta", [], (er, r) => {
		r.forEach(v => {
...
app.post("/stawkiPaliwowe/", (req, res) => {
	let tmp = [];
	db.query("SELECT * FROM stawkiPaliwowe", [], (er, r) => {
		r.forEach(v => {
...
app.post("/aktualizujStawkePaliwa/:token/", (req, res) => {
	if(!req.params.token || (req.body.id === undefined)) {
		res.send({blad: "Brak uprawnień lub identyfikatora"});
		return;
	}
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(rk[0].i < 1) {
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("UPDATE `stawkiPaliwowe` SET `stawka` = ? WHERE `id` = ?", [req.body.nowaStawka, req.body.id], (er, r) => {
			if(er){
...
app.post("/statystykakontofirmowe/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || (rk[0].i <= 0)){
			res.send({blad: "Nieuprawniony"});
			return;
		} else {
			db.query("SELECT * FROM `kontofirmowe` WHERE `kiedy` >= ? AND `kiedy` <= ?", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (er, r) => {
				if(er){
					res.send({blad: "Wystąpił błąd SQL!"});
					return;
				}
				db.query("SELECT SUM(`suma`) as 's' FROM `kontofirmowe` WHERE `kiedy` < ?", [new Date(req.body.czasOd)], (er2, r2) => {
					if(er2){
						res.send({blad: "Wystąpił błąd SQL!"});
						return;
					}
...
app.post("/statystykakadradyspozytor/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || (rk[0].i <= 0)){
			res.send({blad: "Nieuprawniony"});
			return;
		} else {
			db.query("SELECT `kto`, COUNT(CASE WHEN `akcja` > 0 THEN 1 END) as 'zatwierdzone', COUNT(CASE WHEN `akcja` <= 0 THEN 1 END) as 'odrzucone' FROM `dysphistoria` WHERE `kiedy` >= ? AND `kiedy` <= ? GROUP BY `kto`", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (er2, rd) => {
				if(er2){
					res.send({blad: "Wystąpił błąd SQL!"});
					return;
				}
...
app.post("/statystykakadrainstruktor/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT instruktor, SUM(CASE WHEN gra = 1 THEN 1 END) as 'ats', SUM(CASE WHEN gra = 0 THEN 1 END) as 'ets' FROM `uprawnienia` WHERE odkiedy >= ? AND odkiedy <= ? GROUP BY instruktor", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (erb, rb) => {
			if(erb){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.post("/statystykaKierowcySpalanie/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT MIN(G.sp) as 'min', AVG(G.sp) as 'avg', MAX(G.sp) as 'max', G.kto FROM (SELECT spalanie*100/przejechane as 'sp', kto, id FROM trasy WHERE kiedy >= ? AND kiedy <= ? AND zatwierdz = 1 GROUP BY kto, id) as G GROUP BY kto", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (ert, rt) => {
			if(ert){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.post("/statystykaKierowcyPredkosc/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT MIN(vmax) as 'min', AVG(vmax) as 'avg', MAX(vmax) as 'max', kto FROM trasy WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ? GROUP BY kto", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (ert, rt) => {
			if(ert){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.post("/statystykaKierowcyPrzejechane/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT kto, SUM(przejechane) as 'km', SUM(masaladunku) as 'tony' FROM trasy WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ? GROUP BY kto", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (ert, rt) => {
			if(ert){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.post("/statystykaKierowcyZarobki/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT kto, SUM(wlasnyzarobek) as 'zarobek', SUM(premia) as 'premia', SUM(kara) as 'kara' FROM `trasy` WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ? GROUP BY kto", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (ert, rt) => {
			if(ert){
				res.send({blad: "Wystąpił błąd SQL odczytu zarobków i kar z tras."});
				return;
			}
			db.query("SELECT `kto`, SUM(`uprawnienia`.`cena`) as 'c' FROM `uprawnienia` WHERE odkiedy >= ? AND odkiedy <= ? GROUP BY kto", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (eru, ru) => {
				if(eru){
					res.send({blad: "Wystąpił bład SQL odczytu kosztów uprawnień."});
					return;
				}
				db.query("SELECT `komu` as 'kto', SUM(`dodawaniekwoty`.`kwota`) as 'c' FROM `dodawaniekwoty` WHERE kiedy >= ? AND kiedy <= ? GROUP BY `komu`", [new Date(req.body.czasOd), new Date(req.body.czasDo)], async (erd, rd) => {
					if(erd){
...
app.post("/statystykaLogistykaPromy/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT COUNT(idprompociag) as 'ile' FROM trasyprompociag WHERE idtrasa IN (SELECT id FROM `trasy` WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ?)", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (erp, rp) => {
			if(erp){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
			db.query("SELECT COUNT(g.szmeks) as 'unikalnych' FROM (SELECT idprompociag as 'szmeks' FROM trasyprompociag WHERE idtrasa IN (SELECT id FROM `trasy` WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ?) GROUP BY idprompociag) g", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (eru, ru) => {
				if(eru){
					res.send({blad: "Wystąpił błąd SQL"});
					return;
				}
				db.query("SELECT prompociag.nazwa as 'nazwa', COUNT(idprompociag) as 'ile' FROM trasyprompociag LEFT JOIN prompociag ON trasyprompociag.idprompociag = prompociag.id WHERE idtrasa IN (SELECT id FROM `trasy` WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ?) GROUP BY idprompociag ORDER BY ile DESC, prompociag.nazwa ASC LIMIT 10", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (erp2, rp2) => {
					if(erp2){
						res.send({blad: "Wystąpił błąd SQL"});
						return;
					}
...
app.post("/statystykaLogistykaWiele/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT trasy.gra, typyuprawnien.nazwa, COUNT(*) AS liczba_wystapien FROM trasy LEFT JOIN typyuprawnien ON trasy.naczepa = typyuprawnien.id WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ? GROUP BY naczepa, trasy.gra	ORDER BY gra ASC, liczba_wystapien DESC", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (er, r) => {
			if(er){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.post("/statystykaLogistykaPanstwa/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT g.kraj, SUM(g.ile) as 'ile' FROM (SELECT trasy.od, miejscowosci.kraj, COUNT(*) as 'ile' FROM trasy JOIN miejscowosci ON trasy.od = miejscowosci.id WHERE trasy.gra = 0 AND trasy.zatwierdz = 1 AND trasy.kiedy >= ? AND trasy.kiedy <= ? GROUP BY trasy.od) g GROUP BY g.kraj", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (erd, rd) => {
			if(erd){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
			db.query("SELECT g.kraj, SUM(g.ile) as 'ile' FROM (SELECT trasy.do, miejscowosci.kraj, COUNT(*) as 'ile' FROM trasy JOIN miejscowosci ON trasy.do = miejscowosci.id WHERE trasy.gra = 0 AND trasy.zatwierdz = 1 AND trasy.kiedy >= ? AND trasy.kiedy <= ? GROUP BY trasy.do) g GROUP BY g.kraj", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (erc, rc) => {
				if(erc){
					res.send({blad: "Wystąpił błąd SQL"});
					return;
				}
...
app.post("/statystykaLogistykaSpecjalne/:token", (req, res) => {
	db.query("SELECT COUNT(*) as 'i' FROM konta WHERE token = ? AND typkonta <= 2", [req.params.token], (erk, rk) => {
		if(erk || rk[0].i <= 0){
			res.send({blad: "Nieuprawniony"});
			return;
		}
		db.query("SELECT SUM(ladunekADR) as 'adr', SUM(ladunekDelikatny) as 'delikatny', SUM(ladunekGabaryt) as 'gabaryt' FROM `trasy` WHERE zatwierdz = 1 AND kiedy >= ? AND kiedy <= ?", [new Date(req.body.czasOd), new Date(req.body.czasDo)], (er, r) => {
			if(er){
				res.send({blad: "Wystąpił błąd SQL"});
				return;
			}
...
app.get("/typyOsiagniec", (req, res) => {
	db.query("SELECT * FROM typyOsiagniec", (er, r) => {
		if(er){
			res.send({blad: "Wystąpił błąd SQL"});
			return;
		}
...
app.post("/osiagnieciaKierowcy/:kierowca", (req, res) => {
	if(!req.params.kierowca) {
		res.send({blad: "Brak podanej nazwy użytkownika"});
		return;
	}
	db.query("SELECT * FROM osiagniecia WHERE kierowca = (SELECT id FROM konta WHERE login = ?)", [req.params.kierowca], (er, r) => {
		if(er){
			res.send({blad: "Wystąpił błąd SQL"});
			return;
		}
...
db.query("SELECT * FROM `telemetriaPowiazania`", (er, r) => {
	if(er){
...
db.query("SELECT * FROM `telemetriaInformacje`", (er, r) => {
	if(er) {
...
const zapiszTelemetrieBaza = async () => {
	db.query("TRUNCATE TABLE `telemetriaInformacje`");
	telemetriaInformacje.map(kierowca => {
		db.query("INSERT INTO `telemetriaInformacje` (`login`, `ciezarowka`, `naczepa`, `pozycja`, `praca`, `kiedy`) VALUES (?, ?, ?, ?, ?, ?)",
			[
				kierowca.login,
...
const zapiszTelemetriaInne = (token, typ, kwota, akcja) => {
	db.query("SELECT `id`, `login`, `discord` FROM `konta` WHERE `login` = (SELECT `uzytkownik` FROM `telemetriaPowiazania` WHERE `token` = ?) ", [token], async (er, r) => {
		if(er){
		...
		if(r.length > 0){
			db.query("INSERT INTO `telemetriaPozostale` (kto, typ, kwota, kiedy, custom) VALUES (?, ?, ?, ?, ?)", [r[0].id, typ, kwota, new Date(), akcja], async (er2, r2) => {
				if(er2){
...
let infoModyfikacje = [];
db.query("SELECT * FROM `telemetriaModyfikacje`", (er, r) => {
	if(er) {
...
const zapiszTelemetrieModyfikacje = async () => {
	db.query("TRUNCATE TABLE `telemetriaModyfikacje`");
	infoModyfikacje.map(kierowca => {
		db.query("INSERT INTO `telemetriaModyfikacje` (`login`, `gra`, `promods`) VALUES (?, ?, ?)",
			[
				kierowca.login,
				kierowca.gra,
				kierowca.promods
...
io.on('connection', (socket) => {
	...
	socket.on("disconnect", () => {
		console.log("socket: ktos sie rozlaczyl");
	});
	...
	socket.on("modyfikacje", async (msg) => {
		...
		if(tmpDane.blad){
			const embed1 = new EmbedBuilder().setFooter({text: "System The Boss Spedition"}).setTimestamp().setTitle(`Odczyt modyfikacji`)
				...
				.setThumbnail('https://system.thebossspedition.pl/img/logoglowna3.png');
			await dcbot.channels.cache.get(process.env.CHANNEL_TELEMETRIA).send({embeds: [embed1]});
			db.query("SELECT discord FROM konta WHERE login = ?", [tmpLogin], async (er, r) => {
				if(r.length > 0){
					if(r[0].discord){
	...
	socket.on("paliwo", async (msg) => {
		...
		db.query("SELECT `id`, `login`, `discord` FROM `konta` WHERE `login` = (SELECT `uzytkownik` FROM `telemetriaPowiazania` WHERE `token` = ?) ", [tmpObj.token], async (er, r) => {
			if(er){
			...
			if(r.length > 0){
				db.query("INSERT INTO `telemetriaPaliwo` (`kierowca_id`, `pozycjaX`, `pozycjaY`, `litry`, `kwota`, `znizka`, `kraj`, `miasto`, `kiedy`, `gra`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [r[0].id, istotneDane.pozycjaX, istotneDane.pozycjaY, istotneDane.litry, istotneDane.kwota, istotneDane.znizka, najblizsze.kraj, najblizsze.miasto, new Date(istotneDane.kiedy), gra], async (er2, r2) => {
					if(er2){
	...
	socket.on("event", (msg) => {
		...
		if(tmpObj.typ == "player.fined"){
			...
			db.query("SELECT `id`, `login`, `discord` FROM `konta` WHERE `login` = (SELECT `uzytkownik` FROM `telemetriaPowiazania` WHERE `token` = ?) ", [tmpObj.token], async (er, r) => {
				...
				if(r.length > 0){
					db.query("INSERT INTO `telemetriaMandaty` (`kto`, `typ`, `kwota`, `kiedy`) VALUES (?, ?, ?, ?)", [r[0].id, tmpTyp, tmpObj['fine.amount'], new Date(tmpObj.kiedy)], async (er2, r2) => {
						if(er2){
		...
		if(tmpObj.typ == "player.use.train"){
			let wiadomosc = `zapłacił kwotę ${tmpObj['pay.amount']} zł za pociąg z miejscowości ${tmpObj['source.name']} do miejscowości ${tmpObj['target.name']}. DOPISAĆ KRAJE I POLSKIE NAZWY`;
			console.log(wiadomosc);
