...
export default function Logowanie(props){
	...
	const autoryzacja = async (e) => {
		if(login && haslo){
			if(login.length > 2 && login.length < 60 && haslo.length > 3 && haslo.length < 60){
				await Axios.post(gb.backendIP+"login",
				{
					username: login,
					password: haslo,
					dane: window.navigator.userAgent
				}).then((res) => {
					if(!res.data['blad']){
		...
		<div className="logowanieBg">
			<div className="logowanie">
				...
				<div className="logowaniePrawa">
					...
					{blad && <h5>{blad}</h5>}
					<input type="text" name="login" onChange={(e) => { setLogin(e.target.value); setBlad(null); } } placeholder="Nazwa użytkownika"/>
