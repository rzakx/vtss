                            ...
                            {ostatnieTrasy.dane.map((wiersz) => {
                                ...
                                return(
                                    <tr key={"trasa_"+wiersz.id}>
                                        ...
                                        <td>
                                            {skad && (wiersz.gra ? <img title={skad[0]} className="w-5 h-[14px] m-[0_8px_2px_0px]" src={"/img/flagi/usa.png"} /> : <img title={skad[0]} className="w-5 h-[14px] m-[0_8px_2px_0px]" src={"/img/flagi/"+skad[0].toLowerCase().replaceAll("ó", "o").replaceAll("ń", "n").replaceAll("ł", "l").replaceAll(" ", "").replaceAll("ś", "s").replaceAll("ę", "e").replaceAll("ż", "z").replaceAll("ą", "a").replaceAll("ź", "z").replaceAll("ć", "c")+".png"} />)}
                                            <b>{skad ? skad[1] : "???"}</b>
                                            <HiArrowNarrowRight />
                                            {dokad && (wiersz.gra ? <img title={dokad[0]} className="w-5 h-[14px] m-[0_8px_2px_5px]" src={"/img/flagi/usa.png"} /> : <img title={dokad[0]} className="w-5 h-[14px] m-[0_8px_2px_5px]" src={"/img/flagi/"+dokad[0].toLowerCase().replaceAll("ó", "o").replaceAll("ń", "n").replaceAll("ł", "l").replaceAll(" ", "").replaceAll("ś", "s").replaceAll("ę", "e").replaceAll("ż", "z").replaceAll("ą", "a").replaceAll("ź", "z").replaceAll("ć", "c")+".png"} />)}
                                            <b>{dokad ? dokad[1] : "???"}</b>
                                        </td>
                                        <td>{wiersz.ladunek}</td>
                                        <td><a href={"/profil/"+autor}><img className="inline rounded-full align-middle mr-5 w-[45px] h-[45px]" src={"/img/"+uzytkownicy.dane.find(user => user.id === wiersz.kto).awatar} />{autor}</a></td>
    ...
    const odrzucanie = () => {
        return(
            <div className="akcjaDyspozytor wejscieSmooth">
                ...
                <div>
                    <span>Powód:</span><br />
                    <textarea className="text-black" onChange={ (e) => setDaneTrasy({...daneTrasy, powododrzucenia: e.target.value})} />
                </div>
                <div>
                    <span>Zezwolić na poprawę trasy?</span>
                    <select className="text-black" value={daneTrasy.dozwolPoprawe ? daneTrasy.dozwolPoprawe : 1} onChange={(e) => setDaneTrasy({...daneTrasy, dozwolPoprawe: e.target.value})}>
                        <option value={null} />
                ...
                <div style={{marginTop: '40px'}}>
                    <a onClick={() => setDaneTrasy({...daneTrasy, odrzucanie: false, powododrzucenia: null, dozwolPoprawe: null})}><FaShare className="inline" /> Wróć</a>
                    <a onClick={() => decyzja(false)}><FaTimes className="inline" /> Odrzuć</a>
                </div>
            </div>
        )
    };
    ...
    const zatwierdzanie = () => {
        ...
        return(
            <div className="akcjaDyspozytor wejscieSmooth">
                ...
                : "" }
                <div style={{marginTop: '40px'}}>
                    <a onClick={() => setDaneTrasy({...daneTrasy, zatwierdzanie: false, nakladanaGrzywna: null})}><FaShare className="inline" /> Wróć</a>
                    <a onClick={() => decyzja(true)}><FaCheck className="inline" /> Zatwierdź</a>
                </div>
            </div>
        );
    };
    ...
    const pokazHistorie = () => {
        return(
            <div className="dyspHistoria wejscieSmooth">
                ...
                [&_tr:hover_td]:text-[#ddd] [&_svg]:inline [&_svg]:m-[0_6px_-2px_11px] [&_svg]:text-[dodgerblue]
                [&_img]:inline [&_img]:rounded-[0] [&_img]:align-middle
                [&_td_b]:font-medium [&_td_b]:text-white [&_td_b]:text-[0.85rem]">
                    <tbody>
                                ...
                                <tr key={`dyspH_${wiersz.trasa}_${new Date(wiersz.kiedy).getTime()}`}>
                                    ...
                                    <td>
                                        <a href={"/profil/"+uzytkownicy.dane.find(user => user.id === wiersz.kto).login}>
                                            <img className="rounded-full align-middle mr-5 w-[45px] h-[45px]" src={"/img/"+uzytkownicy.dane.find(user => user.id === wiersz.kto).awatar} /> 
			...
			<div className="srodekekranu">
                <div className="glowna" style={{minHeight: '500px'}}>
                    <div className="przegladajTrasy" style={{maxHeight: '70vh', overflowY: 'auto'}}>
                        <h4 className="font-bold p-2.5 text-center">Następujące trasy czekają na rozpatrzenie</h4>
                        {ostatnieTrasy.response ? zwrocTrasy() : (miasta ? dostanTrasy() : dostanMiasta())}
                    </div>
                    {historia.response ? <div className="dyspHistoriaBtn" onClick={() => setHistoria({...historia, wysun: !historia.wysun})}>{historia.wysun ? <FaArrowAltCircleDown className="inline" /> : <FaArrowAltCircleUp className="inline" />} Historia</div> : dostanHistorie() }
                    {historia.wysun && pokazHistorie() }
                    <div className="customDysp">
                        <input type="number" min="1" step="1" placeholder="ID Trasy" id="customIdTrasy"
                        className="leading-none text-[0.85rem]" />
                        <a onClick={() => { document.getElementById("customIdTrasy").value && window.location.assign("/dyspozytornia/"+document.getElementById("customIdTrasy").value); }}><FaSistrix className="inline" /></a>
                    </div>
                </div>
                
            </div>
            {daneTrasy.wyswietl &&
                <div className="flex flex-col items-center justify-center fixed p-5 pl-20 left-0 top-0 right-0 bottom-0 z-[5] bg-black/70 wejscieSmooth">
                    <div className="relative w-[1300px] h-[calc(1310px*9/16)] bg-[#0e0e0e] p-[30px] shadow-[0_0_20px_1px_#000] z-[33] text-[#ddd]
                    dyspozytorniaTrasyForm" style={{height: 'unset', paddingBottom: '5px'}}>
                        ...
                        <div className="gap-3 flex flex-row grow justify-items-start mb-5">
                        ...
                        <div className="gap-3 flex flex-row grow justify-items-start mb-5">
                            ...
                            <div className="formularzDane [&_img]:inline [&_img]:w-5 [&_img]:h-[14px] [&_img]:mr-2">
                        ...
                        <div className="gap-3 flex flex-row grow justify-items-start mb-5">
                            ...
                            <div className="formularzDane">
                                <h3>Uszkodzenia</h3>
                                {daneTrasy.uszkodzenia ? <span title="Wykryto uszkodzenia!" style={{borderLeft: daneTrasy.uszkodzenia && '4px solid crimson'}}><RiAlertFill style={{margin: '-8px 5px -5px -5px', verticalAlign: 'middle', fill: 'crimson', display: 'inline'}}/> {daneTrasy.uszkodzenia.toLocaleString('pl-PL')}%</span>: <span>Brak uszkodzeń</span>}
                            </div>
                            <div className="formularzDane">
