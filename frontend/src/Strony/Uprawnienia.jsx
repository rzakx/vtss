    ...
    const uprawnieniaKoszyk = () => {
        ...
        return nadawane.uprawnienia.map((x, xid) => {
            ...
            <div className="uprBox" key={`uprKoszyk_${xid}_${x.koszt}_${x.upr}`}>
                ...
                <div className="uprBoxLine text-[0.8rem]" style={{borderBottom: 'none'}}>{x.nazwa} ({x.upr})</div>
            </div>
        );
        })
    }
                ...
                <div className="glowna">
					...
					<div className="uprawnieniaOkno">
							...
							<div className="uprawnieniaNadawanieGora">
								<div className="uprawnieniaNadawanieGoraLewa">
									<div className="uprawnieniaNadawanieGoraCol">
										<h3>Kierowca</h3>
										<div className="relative p-[10px_15px] text-[1.1rem] font-bold bg-[#454545] text-[#e5e5e5] text-left min-w-62.5 select-none cursor-pointer">
											...
											<div className="absolute top-full w-full left-0 z-6
                                                            [&_div]:p-[6px_15px] [&_div]:bg-[#bbb] [&_div]:text-[#222] [&_div]:transition-all [&_div]:duration-300 [&_div]:text-shadow-[1px_1px_3px_#666]
                                                            [&_div:hover]:tracking-[1px] [&_div:hover]:bg-[#8d8d8d]
                                                            [&_div:nth-child(even)]:bg-[#eee] [&_div:nth-child(even):hover]:bg-[#d7d7d7]"
                                                style={{display: rozwijane.kierowca ? "" : 'none'}}>
									...
									<div className="uprawnieniaNadawanieGoraCol">
										...
										<div className="relative p-[10px_15px] text-[1.1rem] font-bold bg-[#454545] text-[#e5e5e5] text-left min-w-62.5 select-none cursor-pointer">
											...
											<div className="absolute top-full w-full left-0 z-6
                                                            ...
                                                            [&_div:hover]:tracking-[1px] [&_div:hover]:bg-[#8d8d8d]
                                                            [&_div:nth-child(even)]:bg-[#eee] [&_div:nth-child(even):hover]:bg-[#d7d7d7]"
                                                style={{display: rozwijane.pokrywajacy ? "" : 'none'}}>
						...
						{ historia.usuwane ?
							...
							<table>
                                <thead>
                                    <tr><th>ID</th><th>Posiadacz</th><th>Nadający</th><th>Uprawnienie</th><th>Koszt</th><th>Nadane</th><th>Ważność</th><th>Akcja</th></tr>
                                </thead>
                                <tbody>
                                { historia.response ? historia.dane.map((wiersz) => {
                                    ...
                                    const nadajacy = uzytkownicy.dane.find(u => u.id === wiersz.instruktor);
                                    return(<tr key={`histUpr_${wiersz.id}`}>
                                        <td>{wiersz.id}</td>
                                        <td><a href={"/profil/"+kierowca.login}><img className="h-10 w-10 rounded-[25%]" src={"/img/"+kierowca.awatar} />{kierowca.login}</a></td>
                                        <td>{nadajacy === undefined ? "Nieznane" : <a href={"/profil/"+nadajacy.login}><img className="h-10 w-10 rounded-[25%]" src={"/img/"+nadajacy.awatar} />{nadajacy.login}</a> }</td>
						...
						{ (zakladka == "cennik") &&
                            ...
                            <div className="absolute right-0 bg-card rounded-b-lg border border-input font-bold flex flex-col gap-2 px-4 py-2 text-right">
                                <p>Koszt wydłużenia Szkolenia odpowiada 50% kwoty Licencji.</p>
                                <p>Koszt licencji ATS odpowiada 10% kwoty licencji ETS2.</p>
                            </div>
                            </> }
					</div>
                </div>
