...
import PopupTrasy from "../Komponenty/PopupTrasy";
export default function Trasy(props){
    ...
    const [ komunikat, setKomunikat ] = useState({zawartosc: null, time: null});
            ...
            Axios.post(gb.backendIP+"miasta").then((res2) => {
                Axios.post(gb.backendIP+"typyNaczep").then((res3) => {
                    Axios.post(gb.backendIP+"promy").then((res4) => {
                        ...
                        setPromy(res4.data['dane'].sort((a, b) => a.nazwa.localeCompare(b.nazwa)));
                    });
                });
            });
    };
    ...
    const oddawanieTrasy = (gra, daneinc) => {
        ...
        if(daneinc){
            ...
            let krajdo;
            if(gra){
                krajod = miasta.find((w) => w.id === dane.od) ? miasta.find((w) => w.id === dane.od).kraj : null;
                krajdo = miasta.find((w) => w.id === dane.do) ? miasta.find((w) => w.id === dane.do).kraj : null;
            } else {
                krajod = miasta.find((w) => w.id === dane.od) ? miasta.find((w) => w.id === dane.od).kraj : null;
                krajdo = miasta.find((w) => w.id === dane.do) ? miasta.find((w) => w.id === dane.do).kraj : null;
            }
    ...
    const zwrocTrasy = () => {
        if(ostatnieTrasy.dane){
            return(
                ...
                [&_tr:hover_td]:bg-[#1c1c1c_!important]
                [&_tr:nth-child(odd)_td]:bg-[#121212] [&_tr:nth-child(even)_td]:bg-[#161616]
                [&_tr:hover_td]:text-[#ddd] [&_svg]:inline [&_svg]:m-[0_6px_-2px_11px] [&_svg]:text-[dodgerblue]
                [&_img]:inline [&_img]:rounded-[0] [&_img]:align-middle
                [&_td_b]:font-medium [&_td_b]:text-white [&_td_b]:text-[0.85rem]                wejscieSmooth">
                    <tbody>
                        ...
                        {ostatnieTrasy.dane.map((wi) => {
                            ...
                            let status;
                            let akcja;
                            let skad = miasta.find((w) => w.id === wiersz.od) || null;
                            let dokad = miasta.find((w) => w.id === wiersz.do) || null;
                            ...
                            <tr key={"trasa_"+wiersz.id}>
                                <td>{wiersz.id}</td>
                                <td>{wiersz.gra ? <img className="w-[45px] h-[45px] inline" src={"/img/trasaats.png"}/> : <img className="w-[45px] h-[45px] inline" src={"/img/trasaets.png"}/>}</td>
                                ...
                                <td>
                                    {skad && (wiersz.gra ? <img className="w-5 h-[14px] m-[0_8px_2px_0px]" title={skad.kraj} src="/img/flagi/usa.png" /> : <img title={skad.kraj} className="w-5 h-[14px] m-[0_8px_2px_0px]" src={"/img/flagi/"+skad.kraj.toLowerCase().replaceAll("ó", "o").replaceAll("ń", "n").replaceAll("ł", "l").replaceAll(" ", "").replaceAll("ś", "s").replaceAll("ę", "e").replaceAll("ż", "z").replaceAll("ą", "a").replaceAll("ź", "z").replaceAll("ć", "c")+".png"} /> )}
                                    <b>{skad ? skad.miasto : "???"}</b>
                                    <HiArrowNarrowRight />
                                    {dokad && (wiersz.gra ? <img className="w-5 h-[14px] m-[0_8px_2px_5px]" title={dokad.kraj} src="/img/flagi/usa.png" /> : <img title={dokad.kraj} className="w-5 h-[14px] m-[0_8px_2px_5px]" src={"/img/flagi/"+dokad.kraj.toLowerCase().replaceAll("ó", "o").replaceAll("ń", "n").replaceAll("ł", "l").replaceAll(" ", "").replaceAll("ś", "s").replaceAll("ę", "e").replaceAll("ż", "z").replaceAll("ą", "a").replaceAll("ź", "z").replaceAll("ć", "c")+".png"} /> )}
                                    <b>{dokad ? dokad.miasto : "???"}</b>
                                </td>
            ...
            if(ostatnieTrasy.blad){
                ...
                [&_tr_td:first-child]:text-center [&_tr_th:first-child]:text-center [&_tr_th:nth-child(2)]:text-center
                [&_tr:nth-child(odd)_td]:bg-[#121212] [&_tr:nth-child(even)_td]:bg-[#161616]
                [&_tr:hover_td]:bg-[#1c1c1c] [&_tr:hover_td]:text-[#ddd] [&_td_b]:font-medium [&_td_b]:text-white [&_td_b]:text-[0.85rem] wejscieSmooth">
                    <thead>
                        <tr><th>ID</th><th>Gra</th><th>Data</th><th>Lokalizacja</th><th>Ładunek</th><th>Status</th><th>Akcja</th></tr>
                    </thead>
                    <tbody>
    ...
    const ustawKomunikat = (zawartosc) => {
        setKomunikat({zawartosc: zawartosc, time: Date.now()});
        return;
    };
    ...
    const oddajNowa = () => {
        ...
        if(!daneTrasy.kiedy) {
            ustawKomunikat("Nie ustawiłeś daty oddania raportu!");
            return;
        }
        if(new Date(daneTrasy.kiedy).getTime() < (Date.now() - 25 * 60 * 60 * 1000)) {
            //25 godzin do tyłu
            ustawKomunikat("Data raportu może być maksymalnie do 25 godzin wstecz!");
            return;
        }
        if(!daneTrasy.typserwera) {
            ustawKomunikat("Nie wybrano typu serwera!");
            return;
        }
        if(!daneTrasy.typzlecenia) {
            ustawKomunikat("Nie wybrano typu zlecenia!");
            return;
        }
        if(!daneTrasy.ladunek) {
            ustawKomunikat("Nie uzupełniono ładunku!");
            return;
        }
        if(!daneTrasy.naczepa) {
        ...
        const czas = new Date(daneTrasy.kiedy).toISOString();
    ...
    const poprawTrase = () => {
        ...
        if(daneTrasy.zatwierdz == 0 && (new Date(daneTrasy.kiedy).getTime() < (Date.now() - 25 * 60 * 60 * 1000))) {
            //jesli jest oczekujaca to max 25 godzin do tyłu
            ustawKomunikat("Data raportu może być maksymalnie do 25 godzin wstecz!");
            return;
        }
        if(daneTrasy.typserwera === undefined) {
            ustawKomunikat("Nie wybrano typu serwera!");
            return;
        }
        if(daneTrasy.typzlecenia === undefined) {
            ustawKomunikat("Nie wybrano typu zlecenia!");
            return;
        }
        if(daneTrasy.ladunek === undefined) {
            ustawKomunikat("Nie uzupełniono ładunku!");
            return;
        }
        if(daneTrasy.naczepa === undefined) {
            ustawKomunikat("Nie wybrano typu naczepy!");
            return;
        }
        if(daneTrasy.od === undefined) {
            ustawKomunikat("Nie wybrano miejsca rozpoczęcia trasy!");
            return;
        }
        if(daneTrasy.do === undefined) {
            ustawKomunikat("Nie wybrano miejsca zakończenia trasy!");
            return;
        }
        if(daneTrasy.masaladunku === undefined) {
            ustawKomunikat("Nie wypełniono masy ładunku!");
            return;
        }
        if(daneTrasy.uszkodzenia === undefined) {
            ustawKomunikat("Nie uzupełniono uszkodzeń!");
            return;
        }
        if(daneTrasy.spalanie === undefined) {
            ustawKomunikat("Nie uzupełniono ilości wykorzystanego paliwa!");
            return;
        }
        if(daneTrasy.paliwo === undefined) {
            ustawKomunikat("Nie uzupełniono kosztu tankowanego paliwa!");
            return;
        }
        if(daneTrasy.przejechane === undefined) {
            ustawKomunikat("Nie uzupełniono pokonanego dystansu!");
            return;
        }
        if(daneTrasy.vmax === undefined) {
            ustawKomunikat("Nie uzupełniono prędkości maksymalnej!");
            return;
        }
        if(daneTrasy.zarobek === undefined) {
            ustawKomunikat("Nie uzupełniono zarobku całkowitego!");
            return;
        }
        if(!(Number(daneTrasy.zarobek) > 0)) {
    ...
    return(
        <>
			...
			<div className="srodekekranu">
                <div className="glowna">
                    <div className="p-2.5 relative">
                        <h4 className="text-center m-[5px_0_10px] font-semibold">Twoje ostatnie trasy</h4>
            ...
            {daneTrasy.wyswietl &&
                <div className="flex flex-col fixed p-5 pl-20 left-0 top-0 right-0 bottom-0 z-[5] items-center justify-center bg-black/70
                wejscieSmooth">
                    <div className="relative w-[1300px] h-[calc(1310px*9/16)] bg-[#0e0e0e] p-[30px] z-[33] text-[#ddd] shadow-[0_0_20px_1px_#000]">
                        {komunikat.zawartosc ?
                            <PopupTrasy zawartosc={komunikat.zawartosc} time={komunikat.time} dlugosc={4000} wygasl={() => { setKomunikat({zawartosc: null}); setDaneTrasy({...daneTrasy, trwaOddawanie: false})}} />
                        : ""}

                        { (daneTrasy.id && ((daneTrasy.zatwierdz === 1) || (daneTrasy.zatwierdz === 2))) ?
                        <div className="absolute left-0 bottom-full text-shadow-[1px_1px_3px_#222] text-[0.9rem] p-[15px] bg-[#0e0e0e] text-[#eee] flex flex-col items-center">
                        ...
                        { daneTrasy.pokazZdj && <div
                        className="absolute left-0 right-0 -bottom-10 top-0 bg-[#0c0c0c] cursor-zoom-out bg-contain bg-no-repeat bg-center z-[99]"
                        style={{backgroundImage: `url(${daneTrasy.pokazZdj})`}} onClick={() => setDaneTrasy({...daneTrasy, pokazZdj: null})}/> }
                        <div className="trasaOpcje">
                            { daneTrasy.id ?
                                ...
                                <>
                                    ...
                                    { (!blokada.blokada || (daneTrasy.zatwierdz === 2 && daneTrasy.dozwolpoprawke)) && ( daneTrasy.trwaOddawanie ? <a>Trwa oddawanie...</a> : <a onClick={() => { poprawTrase(); setDaneTrasy({...daneTrasy, trwaOddawanie: true}); }}><FaPen /> Popraw</a>) }
                                </>
                                :
                            ...
                            <>
                                ...
                                { !blokada.blokada && ( daneTrasy.trwaOddawanie ? <a>Trwa oddawanie...</a> : <a onClick={() => { oddajNowa(); setDaneTrasy({...daneTrasy, trwaOddawanie: true}); }}><FaShare /> Oddaj</a>) }
                            </>
                            }
                        </div>
                        <div className="gap-3 flex flex-row grow justify-between mb-5">
                        ...
                        <div className="gap-3 flex flex-row grow justify-between mb-5">
                            <div className="formularzDane">
                                <h3>Ładunek</h3>
                                <input style={{borderLeft: !daneTrasy.ladunek && '4px solid crimson'}} name="ladunek" autoComplete="on" placeholder="----" type="text" value={daneTrasy.ladunek} onChange={(e) => setDaneTrasy({...daneTrasy, ladunek: e.target.value})}/>
                            </div>
                            <div className="formularzDane">
                                <h3>Typ naczepy</h3>
                            ...
                            <div className="formularzDane">
                                ...
                                <select style={{borderLeft: !daneTrasy.od && '4px solid crimson'}} value={daneTrasy.od} onChange={(e) => setDaneTrasy({...daneTrasy, od: e.target.value})}>
                                    <option value={null}>----</option>
                                    { miasta.map(i => { if(i.kraj === daneTrasy.krajOd) { return <option key={`miastoOd_${i.id}`} value={i.id}>{i.miasto}</option>}}) }
                                </select>
                                : "" }
                            </div>
                            <div className="formularzDane">
                                ...
                                <select style={{borderLeft: !daneTrasy.do && '4px solid crimson'}} value={daneTrasy.do} onChange={(e) => setDaneTrasy({...daneTrasy, do: e.target.value})}>
                                    <option value={null}>----</option>
                                    { miasta.map(i => { if(i.kraj === daneTrasy.krajDo) { return <option key={`miastoDo_${i.id}`} value={i.id}>{i.miasto}</option>}}) }
                                </select>
                                : "" }
                            </div>
                        </div>
                        ...
                        <div className="gap-3 flex flex-row grow justify-between mb-5">
                            ...
                            <div className="formularzDane">
                                <h3>Liczba promów</h3>
                                <input type="number" step="1" min="0" max="5" value={daneTrasy.liczbapromow} onChange={(e) => zmianaLiczbyPromow(e.target.value)}/>
                            </div>
                        </div>
                        <div className="gap-3 flex flex-row grow justify-between mb-5">
                            <div className="formularzDane">
                        ...
                        <div className="gap-3 flex flex-row grow justify-between mb-5">
                            ...
                            <div className="formularzDane" style={{maxWidth: 'unset'}}>
                                ...
                                <>
                                    <label htmlFor="uploadZdjecia" style={{marginTop: 0, borderRadius: 0}} className={daneTrasy.noweZdj ? "uploadZdjeciaTak" : "uploadZdjeciaNie"}>{daneTrasy.noweZdj ? "Wybrano zdjęcie..." : "Nowe zdjęcie?"}</label>
                                    <input id="uploadZdjecia" type="file" style={{width: '100%', padding: '10px'}} accept="image/*" onChange={(e) => noweZdjecia(e.target.files)} multiple="multiple" hidden={true}/>
                                </>
                                }
                            </div>
                            <div className="formularzDane" />
